...
/*start postfb*/
FacebookSession::setDefaultApplication('815846801863198', '5a566677dd75b8612a8d0b80ff0ef3f1');
$access_token="CAALmAiLQ8h4BAJHYKEufrBHzkvw0hS77hbEebf70A4uMoXo1GUaERIZABELStIYQmTdjzchLsfG6eQKMPy3RktLB2qW0OZBULZByvElCoxZAcZBgJjZBIpycJ3N5kOCuySHbZBF0kfpTGZAfx2mDi8U17AFZC75aipR1bHJQQWjqGAQjIGv8ghVIM";
...
if($tid%3==0)
{
	$shorten="http://bit.ly/1QA0I44";
}
else if($tid%3==1)
{
	$shorten="http://goo.gl/nU1ufj";
}
else
{
	$shorten="http://tinyurl.com/h52ehux";
}
$page_post = (new FacebookRequest( $session, 'POST', '/'. $page_id .'/feed', array(
    'access_token' => $access_token,
    'message' => "#靠北指考".$tid."       發文請至    \n".$shorten."\n".$new_thread['message'],
	'picture' =>$pic
  ) ))->execute()->getGraphObject()->asArray();
$arrid=preg_split('/[_]/', $page_post['id']);
			$addfacebookbid = array("facebookbid" => $arrid[1]);
			$db->update_query("threads", $addfacebookbid, "tid='{$tid}'");
  $page_post = (new FacebookRequest( $session, 'POST', '/'.$arrid[1] .'/comments', array(
    'message' => "本篇回文網址   "."http://xdchat.tw/mybb/thread-".$tid.".html#quickreply_e",
  ) ))->execute()->getGraphObject()->asArray();
  echo "本篇網址   "."<a href=\"https://www.facebook.com/permalink.php?story_fbid=".$arrid[1]."&id=1458930391069282\">點我前往</a></br>";
  echo "本篇回文網址   "."<a href=\"http://xdchat.tw/fbphplogin/rep.php?bid=".$arrid[1]."\">點我前往</a></br>";
}
else
{
	echo "Error Occured";
